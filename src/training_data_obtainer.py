...
class GetTweets:
  ...
  def build_query_to_search_by_keywords(self, keywords_first_list: List[str], keywords_second_list: List[str]) -> str:
  ...
  def get_tweets(self, query_list: List[str], limit_per_query_search: int, stance:str = None) -> pd.DataFrame:
    ...
    tweets_df = pd.DataFrame(all_tweets, columns = ['tweet_id', 'date', 'author_id', 'text', 'like_count', 'impression_count','retweet_count', 'quote_count'])
