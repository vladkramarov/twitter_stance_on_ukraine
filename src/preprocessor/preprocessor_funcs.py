...
def remove_emojis(tweet: str) -> str:
    ...
    ascii_characters_only = [
        word.encode("ascii", "ignore").decode("ascii") for word in tweet.split()
    ]
    return " ".join(ascii_characters_only).strip()


def drop_short_tweets(
    dataset: pd.DataFrame, text_feature: str = TEXT_COLUMN
) -> pd.DataFrame:
