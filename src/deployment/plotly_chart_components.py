...
def ridge_plot(
    chart_data, metric_to_display="avg_likes_per_post", metric_label="Likes"
):
    fig = go.Figure()
    for i in color_map:
        fig.add_trace(
            go.Violin(
                x=chart_data.loc[chart_data["label"] == i][metric_to_display],
    ...
    fig.update_layout(
        title={
            "text": f"Ridge Plot of Average {metric_label} per Tweet",
            "x": 0.47,
            "yanchor": "top",
        }
    )
