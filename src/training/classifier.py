...
class Classifier:
    ...
    def best_model_checkpoint(self):
        return tf.keras.callbacks.ModelCheckpoint(
            self.best_model_checkpoint_path, monitor="val_loss", save_best_only=True
        )
    ...
    def fit(
        ...
        history = self.model.fit(
            x=[self.train_ids, self.train_attention],
            y=self.train_labels,
...
class Evaluator:
    ...
    def __init__(
        ...
        sanity_dataset: pd.DataFrame = core.SANITY_CHECK_DATAFRAME,
    ):
    ...
    def classify_test_data(self):
        predicted_labels = self.trained_classifier.predict(
            [self.test_ids, self.test_attention]
        )
        self.predicted_labels = predicted_labels

    def classify_sanity_check_dataset(self):
        sanity_ids, sanity_attention = pf.distilbert_tokenizer(
            self.sanity_dataset, "text"
        )
        sanity_predicted_labels = self.trained_classifier.predict(
            [sanity_ids, sanity_attention]
        )
    ...
    def sanity_test_f1_score(self):
        ...
        return f1_score(
            self.sanity_dataset["label"], self.sanity_predicted_labels, average="macro"
        )

    def write_to_tensorboard(self):
        """Writes metrics used for evaluation to a tensorboard log"""
        with tf.summary.create_file_writer(
            self.trained_classifier.log_path
        ).as_default():
