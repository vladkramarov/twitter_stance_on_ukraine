...
train_ids, train_attention, train_labels = pp.prepare_training_data(
    ut.load_data(core.TRAIN_DATASET_DIR)
)
valid_ids, valid_attention, valid_labels = pp.prepare_training_data(
    ut.load_data(core.VALID_DATASET_DIR)
)
test_ids, test_attention, test_labels = pp.prepare_training_data(
    ut.load_data(core.TEST_DATASET_DIR)
)
...
def train_and_evaluate() -> Tuple[ct.Classifier, ct.Evaluator]:
    ...
    classifier = ct.Classifier(
        mc.compile_model(),
        "transformer_conv_3_maxpool_conv5_maxpool_global_max",
        [train_ids, train_attention, train_labels],
        [valid_ids, valid_attention, valid_labels],
    )
    classifier.fit(batch_size=16, epochs=10)
    evaluator = ct.Evaluator(classifier, [test_ids, test_attention, test_labels])
