...
from tensorflow.keras.optimizers.legacy import (
    Adam,
)  # regular Adam optimizer does not run on GPU when using VSCode on Macs
...
def compile_model():
    ...
    attention_masks = Input(
        shape=(core.MAX_LENGTH,), name="attention_mask", dtype="int32"
    )
    embedding_layer = distilbert(input_ids, attention_masks)[0]

    conv_3 = Conv1D(128, kernel_size=3, activation="relu", padding="same")(
        embedding_layer
    )
