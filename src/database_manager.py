...
def write_to_db(
    ...
    connector_func: Callable = connect_to_db,
) -> None:
    """Writes a Pandas DataFrame to a PostreSQL table with the specified name"""
    conn, cursor = connector_func()
    for index, row in dataset_with_labels.iterrows():
        cursor.execute(
            f"""INSERT INTO {table_name} (tweet_id, author_id, text, created_at, like_count, impression_count, retweet_count, quote_count, label) 
...
def check_total_entries(
    table_name: str = core.TABLE_NAME, connector_func: Callable = connect_to_db
) -> int:
    conn, cursor = connector_func()
